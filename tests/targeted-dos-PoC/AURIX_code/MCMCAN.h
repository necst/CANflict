...
/*------------------------------------------------------Macros-------------------------------------------------------*/
/*********************************************************************************************************************/
#define CAN_MESSAGE_ID              (uint32_t)0x38D//(uint32)0x38D//(uint32_t)0x777///(uint32_t)0x77C//(uint32)0x38D           /* Message ID that will be used in arbitration phase    */
...
#define CAN_BAUDRATE               1000000//120000//1000000
...
typedef struct
{
    ...
    IfxCan_Can_Pins pinConfig;
    boolean rcv;
    IfxCan_LastErrorCodeType rcvError;
} McmcanType;

// Node structs
#define CAN0_SRC (&(g_mcmcan0.canSrcNode))
#define CAN0_DST (&(g_mcmcan0.canDstNode))
#define CAN1_SRC (&(g_mcmcan1.canSrcNode))
#define CAN1_DST (&(g_mcmcan1.canDstNode))

// CAN pin structs (pin, index, alternate function)
#define CAN0_RX (&IfxCan_RXD00B_P20_7_IN)
#define CAN0_TX (&IfxCan_TXD00_P20_8_OUT)
#define CAN1_RX (&IfxCan_RXD10C_P23_0_IN)
#define CAN1_TX (&IfxCan_TXD10_P23_1_OUT)

//#define READING
#define WRITING

extern McmcanType g_mcmcan0, g_mcmcan1;

/*********************************************************************************************************************/
/*-----------------------------------------------Function Prototypes-------------------------------------------------*/
/*********************************************************************************************************************/
/// \brief Init CAN0 and CAN1
void initSingleMcmcan(McmcanType *can, uint8_t canId, boolean loop);
void initMcmcan(void);

/// \brief Transmit a packet with either CAN0 or CAN1
void transmitCanMessage(unsigned ID);
void transmitFullCanMessage(unsigned canId, uint32 msgId, uint8* payload, uint8 size);
void transmitCanMessageByte(unsigned canId, uint8_t lastByte);

/// \brief if CAN<id> has received something, print it
boolean printRcvMsg(unsigned id);

/// \brief if CAN<id> has an error
boolean printLastError(unsigned id);

void printNReceived(void);
